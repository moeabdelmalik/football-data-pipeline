...
def settings(tmp_path):
    return Settings(api_key="KEY", gcp_project_id=None, gcs_raw_bucket=None, local_raw_dir=tmp_path)


@responses.activate
def test_run_lands_one_file_per_task(tmp_path, config):
    responses.get("https://api.test/json/KEY/events.php", json={"events": [{"idEvent": "1"}]})

    outcomes = run(
        config=config, settings=settings(tmp_path), destination="local",
...
@responses.activate
def test_one_failing_task_does_not_stop_the_others(tmp_path, config):
...
@responses.activate
def test_rerunning_a_task_is_idempotent(tmp_path, config):
...
@responses.activate
def test_empty_result_still_writes_a_file(tmp_path, config):
...
def test_dry_run_makes_no_http_calls_and_writes_nothing(tmp_path, config, caplog):
    with responses.RequestsMock():   # any HTTP call would raise here
        outcomes = run(
            config=config, settings=settings(tmp_path), destination="local", dry_run=True,
        )
    assert outcomes == []
...
@responses.activate
def test_main_exit_code_reflects_failure(tmp_path, monkeypatch, config):
